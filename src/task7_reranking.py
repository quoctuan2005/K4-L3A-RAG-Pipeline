...
def rerank_rrf(
    ...
    for ranked_list in ranked_lists:
        for rank, item in enumerate(ranked_list, 1):
            ...
            # Giữ bản ghi đầu tiên gặp được; các list sau chỉ đóng góp thứ hạng.
            items.setdefault(item_id, item)

    ranked_ids = sorted(scores, key=lambda item_id: scores[item_id], reverse=True)
    results = []
    for item_id in ranked_ids[:top_k]:
        # Copy trước khi ghi đè để hai ranked list đầu vào giữ nguyên cosine
        # score gốc — Task 9 còn cần đọc score đó để quyết định fallback.
